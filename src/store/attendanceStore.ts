	...
	saveAttendanceStatus: async (lessonId, studentId, status) => {
		...
		try {
			...
			const { data: upsertedData, error } = await supabase
				...
				.upsert(
					...
					{ onConflict: 'lesson_id,student_id', // Assumes unique constraint on these two
					  ignoreDuplicates: false } // Ensure it updates existing record
				)
			...
			const updatedAttendance = attendance.map(a => 
				a.lesson_id === lessonId && a.student_id === studentId ? { ...a, status: status, id: upsertedData.id } : a
			);
			
			// Check if the record was updated or added
			const recordExists = attendance.some(a => a.lesson_id === lessonId && a.student_id === studentId);
