...
import { FiFileText, FiCalendar, FiClock, FiSearch, FiChevronDown, FiCheckCircle, FiPaperclip, FiDownload, FiX, FiVideo, FiFile } from 'react-icons/fi';
import Card from '../../components/common/Card';
import { useAuth } from '../../contexts/AuthContext';
import { Assignment, AttachmentFile, getAssignmentsForSingleStudent } from '../../services/assignmentService';
...
// Extend the Assignment interface to include file_url
...
// Updated Status Color logic (remove theme param)
const getStatusColor = (status: string) => { 
  // Access theme via props in the styled component itself
  switch(status?.toLowerCase()) {
    case 'completed':
      return '#4caf50'; // Use fallbacks directly for now
    case 'in-progress':
      return '#2196f3';
    case 'overdue':
      return '#f44336';
...
const Assignments: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setAssignments(assignmentsData as ExtendedAssignment[]);
  ...
  const getStatusText = (status: string, dueDate: string | undefined): string => {
    ...
    switch(status) {
      case 'completed':
        return 'Completed';
      case 'in-progress': {
        const days = getLocalDaysRemaining(dueDate);
  ...
  const getFileIcon = (fileUrl: string | null | undefined) => {
    ...
    if (fileUrl.includes('.mp4') || fileUrl.includes('.mov') || fileUrl.includes('.avi')) return <FiVideo size={20} />;
  ...
  const getFileName = (fileUrl: string | null | undefined) => {
    ...
    try {
      ...
      if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}[_-]/.test(decodedName)) {
        decodedName = decodedName.replace(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}[_-]/, '');
      } 
              ...
              {assignment.status === 'completed' && (
                <CompletedSection>
                  <CompletedHeader>
                    ...
                    <span>Completed on {formatDate(assignment.submission_date || assignment.updated_at)}</span>
                  </CompletedHeader>
                  {typeof assignment.grade === 'number' && (
                    <GradeDisplay>
                      <GradeLabel>Grade:</GradeLabel>
                      <GradeValue $score={assignment.grade}>{assignment.grade}%</GradeValue>
                    </GradeDisplay>
                  )}
                  {assignment.feedback_provided && (
                    <FeedbackButton>View Feedback</FeedbackButton>
                  )}
                </CompletedSection>
              )}
            </AssignmentContent>
      ...
      >
        {selectedAssignment && (
          <ModalContent>
            <ModalHeader>
              <div>
                <CourseName>{selectedAssignment.subject}</CourseName>
            ...
            <ModalBody>
              <StatusSection>
                <AssignmentStatus $status={selectedAssignment.status}>
                  <span>{getStatusText(selectedAssignment.status, selectedAssignment.due_date)}</span>
                </AssignmentStatus>
              </StatusSection>
              
              <DetailSection>
                <DetailSectionTitle>Description</DetailSectionTitle>
                <DetailContent>{selectedAssignment.description}</DetailContent>
              </DetailSection>
              ...
              {selectedAssignment?.file_url && (
                <DetailSection>
                  ...
                  <MaterialsList>
                    <MaterialItem>
                      <MaterialIcon>
                        {getFileIcon(selectedAssignment.file_url)}
                      </MaterialIcon>
                      <MaterialInfo>
                        <MaterialTitle>{getFileName(selectedAssignment.file_url)}</MaterialTitle>
                        <MaterialDescription>Assignment material from your teacher</MaterialDescription>
                      </MaterialInfo>
                      <MaterialActions>
                        <MaterialButton 
                          onClick={() => {
                            if (selectedAssignment.file_url) {
                              window.open(selectedAssignment.file_url, '_blank');
                            }
                          }}>
                          View
                        </MaterialButton>
                        <MaterialButton 
                          onClick={() => {
                            if (selectedAssignment.file_url) {
                              downloadFile({ 
                                id: '1', 
                                name: getFileName(selectedAssignment.file_url), 
                                size: 'Unknown', 
              ...
              <DetailSection>
                <DetailSectionTitle>Due Dates</DetailSectionTitle>
                <DateDetails>
                  <DateItem>
                    <FiCalendar size={16} />
                    <span>Assigned: {formatDate(selectedAssignment.created_at)}</span>
                  </DateItem>
                  <DateItem>
                    <FiClock size={16} />
                    <span>Due: {formatDate(selectedAssignment.due_date)} at {formatTime(selectedAssignment.due_date)}</span>
                  </DateItem>
                </DateDetails>
              </DetailSection>
              ...
              {selectedAssignment.attachments && selectedAssignment.attachments.length > 0 && (
                <DetailSection>
                  <DetailSectionTitle>
                    <FiPaperclip size={16} />
                    <span>Attachments ({selectedAssignment.attachments.length})</span>
                  </DetailSectionTitle>
                  <AttachmentsList>
                    {selectedAssignment.attachments.map(attachment => (
                      <AttachmentDownloadItem key={attachment.id}>
                        <div className="file-info">
                          <FiFileText size={16} />
                          <div>
                            <div className="file-name">{attachment.name}</div>
                            <div className="file-meta">{attachment.size}</div>
                          </div>
                        </div>
                        <DownloadButton onClick={() => downloadFile(attachment)}>
                          <FiDownload size={16} />
                          <span>Download</span>
                        </DownloadButton>
              ...
              {selectedAssignment.status === 'completed' && (
                <DetailSection>
                  <DetailSectionTitle>Submission Status</DetailSectionTitle>
                  <CompletedHeader>
                    <FiCheckCircle size={16} color="#4caf50" />
                    <span>Completed on {formatDate(selectedAssignment.submission_date || selectedAssignment.updated_at)}</span>
                  </CompletedHeader>
                  {typeof selectedAssignment.grade === 'number' && (
                    <GradeDisplay>
                      <GradeLabel>Grade:</GradeLabel>
                      <GradeValue $score={selectedAssignment.grade}>{selectedAssignment.grade}%</GradeValue>
                    </GradeDisplay>
                  )}
                </DetailSection>
              )}
            ...
            <ModalFooter>
              ...
              {selectedAssignment.status === 'completed' && selectedAssignment.feedback_provided && (
                <PrimaryButton>
                  View Feedback
                </PrimaryButton>
              )}
...
const AssignmentStatus = styled.span<StatusProps>`
  ...
  font-size: 0.75rem;
  font-weight: 600;
  // Access theme via props here for theme-aware colors
  background-color: ${props => {
    const color = getStatusColor(props.$status); // Call without theme
    // Attempt to use theme color with fallback
    const themeColor = 
        props.$status === 'completed' ? props.theme.colors.success?.[500] : 
        props.$status === 'in-progress' ? props.theme.colors.primary?.[500] : 
        props.$status === 'overdue' ? props.theme.colors.danger?.[500] : 
        props.$status === 'upcoming' ? props.theme.colors.info?.[500] : 
        props.$status === 'pending' ? props.theme.colors.warning?.[500] : 
        props.theme.colors.neutral?.[500];
    return `${themeColor || color}1A`; // Add alpha
  }};
  color: ${props => {
    const color = getStatusColor(props.$status); // Call without theme
    // Attempt to use theme color with fallback
...
const GradeValue = styled.div<GradeValueProps>`
  ...
  color: ${props => {
    ...
    if (props.$score >= 80) return '#8bc34a';
