...
const Grades: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        toast.error(t('studentPanel.grades.messages.loadingError'));
  ...
  const runDiagnostic = async () => {
    if (!user?.id) {
      toast.error(t('studentPanel.grades.diagnostics.loginRequired'));
      return;
    }
    ...
    try {
      ...
      if (results.success) {
        toast.success(t('studentPanel.grades.diagnostics.setupVerified'));
      } else {
        toast.error(t('studentPanel.grades.diagnostics.issuesFound', { message: results.message }));
      }
    } catch (error) {
      console.error('Error running diagnostic:', error);
      toast.error(t('studentPanel.grades.diagnostics.failed'));
    } finally {
  ...
  if (loading) {
    return (
      <LoadingContainer>
        <LoadingSpinner />
        <p>{t('studentPanel.grades.loading')}</p>
      </LoadingContainer>
    );
  }
  ...
  return (
    <GradesContainer>
      <PageHeader>
        <HeaderContent>
          <PageTitle>{t('studentPanel.grades.title')}</PageTitle>
          <PageDescription>{t('studentPanel.grades.description')}</PageDescription>
        </HeaderContent>
      ...
      {diagnosticMode && (
        <DiagnosticsPanel>
          <DiagnosticsHeader>
            <h3>{t('studentPanel.grades.diagnostics.title')}</h3>
          ...
          {diagnosticResults ? (
            <>
              <DiagnosticsSection>
                <h4>{t('studentPanel.grades.diagnostics.tablesStatus')}:</h4>
                <TableGrid>
              ...
              {diagnosticResults.userData && (
                <DiagnosticsSection>
                  <h4>{t('studentPanel.grades.diagnostics.userData')}:</h4>
                  <div>
                    <p><strong>{t('studentPanel.grades.diagnostics.classes')}:</strong> {diagnosticResults.userData.classes?.length || 0}</p>
                    <p><strong>{t('studentPanel.grades.diagnostics.subjects')}:</strong> {diagnosticResults.userData.subjects?.length || 0}</p>
                  </div>
                </DiagnosticsSection>
              )}
            ...
            <p>{t('studentPanel.grades.diagnostics.noInfo')}</p>
          )}
        </DiagnosticsPanel>
      )}
      ...
      <FilterSection>
        <SearchBar>
          ...
          <SearchInput 
            type="text" 
            placeholder={t('studentPanel.grades.searchPlaceholder')} 
            value={searchTerm}
            ...
            <motion.div key={subject.id} variants={cardVariants} layout>
              ...
              >
                ...
                <ViewMoreButton onClick={(e) => {
                  ...
                  <span>{t('studentPanel.grades.viewFullDetails')}</span>
      ...
      {filteredSubjects.length === 0 && (
        <NoSubjectsMessage>
          <FiBook size={40} />
          <h3>{t('studentPanel.grades.noSubjectsFound')}</h3>
          <p>{t('studentPanel.grades.noSubjectsMessage')}</p>
        </NoSubjectsMessage>
      )}
    </GradesContainer>
  );
