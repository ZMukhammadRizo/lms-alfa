...
const StudentDashboard: React.FC = () => {
  ...
  const dashboardStats: DashboardStatCard[] = [
    ...
    { id: 2, title: 'Assignments', value: assignmentCount.toString(), icon: <FiFileText />, color: 'yellow' },
  ];
        ...
        >
          ...
          <ContentCard>
            {assignments.length > 0 ? (
              ...
              <EmptyState>No upcoming assignments</EmptyState>
            )}
          </ContentCard>
        </GridItem>
            ...
            {schedule.length > 0 ? (
              ...
              >
                <ScheduleTime>
                  ...
                  <TimeText>{entry.startTime} - {entry.endTime}</TimeText>
                </ScheduleTime>
                <ScheduleDetails>
            ...
            {grades.length > 0 ? (
              ...
              >
                <GradeHeader>
                  ...
                  <GradeDate>{new Date(grade.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</GradeDate>
                </GradeHeader>
                <GradeTitle>{grade.lessonName}</GradeTitle>
                <GradeValue $grade={grade.score}>
                  {grade.score}
                </GradeValue>
              </GradeItem>
              ))
...
const AssignmentStatus = styled.div<StatusProps>`
  ...
  ${props => {
    switch(props.$status) {
      ...
      default:
        return `
          background-color: ${props.theme.colors.neutral?.[50] || props.theme.colors.background.secondary };
  ...
  ${props => {
    ...
    if (score >= 9) { // A (9-10)
      return `
        ...
        color: ${props.theme.colors.success[500]};
      `;
    } else if (score >= 8) { // B (8-8.9)
      return `
        background-color: ${props.theme.colors.primary[50]};
        color: ${props.theme.colors.primary[500]};
      `;
    } else if (score >= 7) { // C (7-7.9)
      return `
        background-color: ${props.theme.colors.warning[50]};
        color: ${props.theme.colors.warning[500]};
      `;
    } else if (score >= 6) { // D (6-6.9) - Using info or another distinct color
        return ` 
