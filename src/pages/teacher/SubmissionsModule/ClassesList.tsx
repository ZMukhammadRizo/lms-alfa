...
const ClassesList: React.FC = () => {
	...
	const fetchTeacherClasses = useSubmissionsStore(state => state.fetchTeacherClasses)

	useEffect(() => {
		const loadData = async () => {
			if (user?.id) {
				console.log('ClassesList: Loading teacher classes')
				try {
	...
	const handleClassClick = (classId: string) => {
		navigate(`/teacher/submissions/classes/${classId}/students`)
	}
	...
	return (
		<PageContainer>
			<PageHeaderWrapper>
				<PageHeader>
					<HeaderContent>
						<PageTitle>{t('submissions.myClasses')}</PageTitle>
			...
			{filteredClasses.length === 0 ? (
				<EmptyStateContainer>
					...
					<EmptyStateTitle>
						{searchTerm
							? t('submissions.noClassesMatchSearch')
							: t('submissions.noClassesAssigned')}
					</EmptyStateTitle>
					<EmptyStateText>
						{searchTerm
							? t('submissions.tryDifferentClassSearch')
							: t('submissions.noClassesAssignedToTeacher')}
					</EmptyStateText>
				</EmptyStateContainer>
			) : (
				<>
