...
const TeacherAssignmentFiles: React.FC = () => {
	...
	const fetchAssignment = async () => {
		...
		setIsLoading(true)
		try {
			// Fetch assignment data with subject information
        ...
        `
			...
			if (data) {
				...
				// Get subject name from the join result
				if (data.subjects && data.subjects.subjectname) {
					subjectName = data.subjects.subjectname
				} else if (data.subject_id) {
					// If we have subject_id but not the join result, fetch it directly
					try {
				...
				// Process file URLs - handle both string and array formats
				if (data.file_url) {
	...
	const getFileType = (
		url: string
	): 'image' | 'pdf' | 'document' | 'spreadsheet' | 'presentation' | 'text' | 'other' => {
		try {
			if (!url) return 'other'
...
const FileItem = styled.div<{ isActive: boolean }>`
	...
	background-color: ${props =>
		props.isActive ? props.theme.colors.background.tertiary : 'transparent'};
...
export default TeacherAssignmentFiles
