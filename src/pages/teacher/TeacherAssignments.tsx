...
import { 
  ...
  FiMail, FiPaperclip, FiUploadCloud, FiCheck
} from 'react-icons/fi';
...
const TeacherAssignments = () => {
  ...
  const courses = [...new Map(assignments.map(item => [item.classid, { id: item.classid, name: item.class_name }])).values()];
  ...
  const fetchAssignments = async () => {
    ...
    setIsLoading(true);
    try {
      // First, fetch the classes that the teacher is associated with
      const { data: teacherClasses, error: classesError } = await supabase
        .from('classes')
        .select('id, classname')
        .eq('teacherid', user.id); // Assuming 'teacherid' is the field linking teachers to classes
      ...
      // Extract class IDs for the query
      const classIds = teacherClasses.map(c => c.id);
      
      // Set the classes state
      setClasses(teacherClasses);
      
      // Fetch assignments that belong to the teacher's classes
      const { data, error } = await supabase
        ...
        `)
        .in('classid', classIds) // Filter to only include assignments for teacher's classes
        .order('duedate', { ascending: true });

      if (error) throw error;

      // Process assignments and fetch related data where needed
      const enhancedAssignments = await Promise.all((data || []).map(async (assignment) => {
        ...
        if (assignment.createdby && typeof assignment.createdby === 'object' && assignment.createdby.email) {
  ...
  const filteredAssignments = assignments.filter(assignment => {
    ...
    const matchesFilter = 
      ...
      (filter === 'upcoming' && assignment.duedate && new Date(assignment.duedate) > new Date() && assignment.status !== 'completed');
    
    const matchesCourse = 
      !selectedCourse || 
      (selectedCourse && assignment.classid === String(selectedCourse));
