...
import { Card, Container, Row, Col, Button, Badge } from '../../components/ui';
import { PageTitle } from '../../components/common';
import { fetchParentAssignments, Assignment } from '../../services/assignmentService';
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [fetchedChildren, fetchedAssignments] = await Promise.all([
            fetchParentChildren(parentId),
            fetchParentAssignments(parentId)
        ]);
                ...
                <Col lg={8}>
                  <AssignmentHeader>
                    ...
                    <StatusBadge $status={assignment.status as FilterType | 'overdue'}>
                      {getStatusIcon(assignment.status)} {assignment.status.charAt(0).toUpperCase() + assignment.status.slice(1)}
                    </StatusBadge>
                  </AssignmentHeader>
                  <AssignmentSubject>{assignment.className || assignment.subject}</AssignmentSubject>
                  ...
                  <AssignmentMeta>
                    <MetaItem>
                      ...
                      <MetaText>
                        ...
                        <MetaValue>{new Date(assignment.due_date).toLocaleDateString()}</MetaValue>
                      </MetaText>
                    </MetaItem>
                    <MetaItem>
                    ...
                    {assignment.grade && (
                      <MetaItem>
                        <MetaIcon><FiCheck /></MetaIcon>
                        <MetaText>
                          <MetaLabel>Grade</MetaLabel>
                          <MetaValue>{assignment.grade}</MetaValue>
                        </MetaText>
                      </MetaItem>
                    )}
      ...
      >
        {selectedAssignment && (
          <ModalContent>
            <ModalHeader>
              <div>
                <ModalCourseName>{selectedAssignment.className || selectedAssignment.subject}</ModalCourseName>
            ...
            <ModalBody>
              <ModalStatusSection>
                <StatusBadge $status={selectedAssignment.status as FilterType | 'overdue'}>
                  {getStatusIcon(selectedAssignment.status)} {selectedAssignment.status.charAt(0).toUpperCase() + selectedAssignment.status.slice(1)}
                </StatusBadge>
              </ModalStatusSection>

              <ModalDetailSection>
                <ModalDetailSectionTitle>Description</ModalDetailSectionTitle>
                <ModalDetailContent>{selectedAssignment.description || 'No description provided.'}</ModalDetailContent>
              </ModalDetailSection>
              ...
              <ModalDetailSection>
                ...
                <ModalDetailsGrid>
                  <MetaItem>
                      ...
                      <MetaText>
                        <MetaLabel>Due Date</MetaLabel>
                        <MetaValue>{new Date(selectedAssignment.due_date).toLocaleDateString()} at {new Date(selectedAssignment.due_date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</MetaValue>
                      </MetaText>
                    </MetaItem>
                    <MetaItem>
                      <MetaIcon><FiUser /></MetaIcon>
                    ...
                    {typeof selectedAssignment.grade === 'number' && (
                       <MetaItem>
                        ...
                        <MetaText>
                          <MetaLabel>Grade</MetaLabel>
                          <MetaValue>{selectedAssignment.grade}%</MetaValue>
                        </MetaText>
                      </MetaItem>
                    )}
...
const FilterButton = styled.button<FilterButtonProps>`
  ...
  &:hover {
    background-color: ${props => !props.$active ? (props.theme?.colors?.background?.hover || '#f0f0f0') : undefined};
...
const StatusBadge = styled(Badge)<StatusBadgeProps>`
  ...
  background-color: ${props => {
    ...
    switch (props.$status) {
      case 'completed': return colors.success?.[100] || '#e6f7e6';
      case 'pending': return colors.warning?.[100] || '#fff7e6';
      case 'overdue': return colors.danger?.[100] || '#ffe6e6';
      case 'upcoming': return colors.info?.[100] || '#e6f7ff';
      default: return colors.primary?.[100] || '#e6f7ff';
    }
  }};
  color: ${props => {
    const colors = props.theme?.colors || {};
    switch (props.$status) {
      case 'completed': return colors.success?.[800] || '#237804';
      case 'pending': return colors.warning?.[800] || '#ad6800';
      case 'overdue': return colors.danger?.[800] || '#a8071a';
      case 'upcoming': return colors.info?.[800] || '#0050b3';
      default: return colors.primary?.[800] || '#0050b3';
    }
  }};
`;
