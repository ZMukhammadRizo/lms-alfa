...
const formatAttendanceDays = (days: string[]): string => {
  ...
  if (days.length > 2) {
    ...
    if (isConsecutive) {
      return `${dayAbbreviations[dayOrder[indices[0]]]}-${dayAbbreviations[dayOrder[indices[indices.length-1]]]}`;
    }
  }
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <AddButton onClick={onCreateNewSection} style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <FiPlus />
...
const ClassNumberDisplay = styled.span<ClassNumberDisplayProps>`
  font-weight: 700;
  font-size: 20px;
  background: linear-gradient(135deg, ${props => props.color || '#4F46E5'}, ${props => props.color ? adjustColorBrightness(props.color, -15) : '#3730a3'});
...
const Classes: React.FC = () => {
  ...
  const [studentToDeleteFromClass, setStudentToDeleteFromClass] = useState<{id: string, name: string} | null>(null);
  ...
  const handleOpenAddStudentsModal = async () => {
    ...
    try {
      ...
      try {
        ...
        if (rpcError) {
          console.log('Note: Could not register RPC function. This may be normal if you do not have privileges:', rpcError);
        } else {
  ...
  const handleAddStudentsToClass = async (studentIds: string[]) => {
    ...
    // 1. Validate Class ID
    if (!classId || typeof classId !== 'string' || classId.length < 36) { // Basic UUID check
    ...
    for (const studentId of studentIds) {
      if (!studentId || typeof studentId !== 'string' || studentId.length < 36) { // Basic UUID check
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`Successfully added ${successCount} student${successCount !== 1 ? 's' : ''} to class`);
  ...
  const handleViewStudents = async (grade: string, section: string | null) => {
    ...
    try {
      ...
      const transformedStudents = await Promise.all(
        classstudents.map(async (enrollment) => {
          ...
          if (userError || !userData) {
            console.error(`Error fetching user data for student ID ${enrollment.studentid}:`, userError);
            return null;
          }
          ...
          if (subjectsError) {
            console.error(`Error fetching subjects for class ID ${enrollment.classid}:`, subjectsError);
          }
      ...
      const validStudents = transformedStudents.filter((student): student is Student => student !== null);
  ...
  const getUniqueCourses = () => {
    try {
      ...
      const validStudents = students.filter(student => 
        student && student.subjects && Array.isArray(student.subjects)
      );
  ...
  const filteredSections = sections.filter(section => {
    ...
    return (section.name && section.name.toLowerCase().includes(termLower)) ||
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesCourse = courseFilter === 'all' || (
      ...
      student.subjects.some(subject => 
        subject && subject.toLowerCase() === courseFilter.toLowerCase()
      )
    );
    ...
    try {
      ...
      if (existingCategory && existingCategory.length > 0) {
          ...
          if (createCategoryError) {
              ...
              toast.error(`Failed to create category '${categoryNameToFind}'. Error: ${createCategoryError.message}`);
              setIsAddSectionModalOpen(false);
              return;
          }
    ...
    try {
      ...
      setSections(prevSections => prevSections.map(section => 
        section.id === sectionId 
          ? { ...section, name: sectionName, room: room } 
          : section
      ));
  ...
  const handleSaveTeacher = async (sectionId: string, teacherName: string, teacherId?: string) => {
    try {
      console.log(`Assigning teacher ${teacherName} (ID: ${teacherId || 'not provided'}) to section ${sectionId}`);
  ...
  const handleSaveClass = async (
    ...
    try {
      ...
      console.log('Note: Attendance days, times, and status are not being saved to the database because the columns do not exist');
  ...
  return (
    <ClassesContainer>
      ...
      {showStudents ? (
                  ...
                  <StudentsTableRow key={student.id}>
                    ...
                    <StudentsTableCell style={{ width: '12%' }}>
                      <StudentActionButtons>
                        ...
                        <ActionIconButton onClick={() => handleDeleteStudentFromClass(student.id, student.name)} title="Remove from class">
                          <FiTrash2 />
                        </ActionIconButton>
                        {/* Use handleEditStudent for the edit icon */}
                        <ActionIconButton onClick={() => handleEditStudent(student)} title="Change class">
                          <FiEdit />
                        </ActionIconButton>
                      </StudentActionButtons>
                    </StudentsTableCell>
        ...
        <>
          ...
          {filteredClasses.length === 0 && !isLoading && (
            <EmptyState>
              <EmptyStateText>No classes found. Try adjusting your search or filters.</EmptyStateText>
            </EmptyState>
          )}
              ...
              {filteredClasses.map((cls) => (
                ...
                >
                    ...
                    <CardActions onClick={(e) => {
                      ...
                      <AnimatePresence>
                        {isActionsMenuOpen === cls.id && (
                          ...
                          >
                            <ActionButton $isPrimary={true} onClick={(e) => {
                              e.stopPropagation();
                            ...
                            <ActionButton $isPrimary={false} onClick={(e) => {
                              e.stopPropagation();
                  ...
                  <ClassDetails>
                    ...
                    <DetailItem>
                      <FiLayers />
                      <span>{cls.sectionCount} {cls.sectionCount === 1 ? 'Section' : 'Sections'}</span>
                    </DetailItem>
                  </ClassDetails>
                </ClassCard>
              ))}
            ...
            <ClassTable>
              ...
              {filteredClasses.map((cls) => (
                ...
                >
                  ...
                  <div className="days">{cls.sectionCount} {cls.sectionCount === 1 ? 'Section' : 'Sections'}</div>
      ...
      <DeleteConfirmationModal
        ...
        message={`Are you sure you want to remove ${studentToDeleteFromClass?.name || 'this student'} from the class ${selectedSection}? This action cannot be undone.`}
      />
      ...
      <DeleteConfirmationModal
        ...
        message={sectionToDelete ? `Are you sure you want to delete section ${sectionToDelete.name}? This will remove all student-section associations and cannot be undone.` : ''}
      />
  ...
  useEffect(() => {
    const fetchAvailableStudents = async () => {
      ...
      try {
        ...
        const unassignedStudents = allStudentsData.filter(
          user => !assignedStudentIds.has(user.id)
        );
    ...
    <ModalOverlay onClick={onClose}>
      <ModalContent onClick={e => e.stopPropagation()}>
                ...
                {filteredStudents.map(student => (
                  ...
                  >
                    ...
                    <StudentInfo>
                      ...
                      <ModalStudentEmail>
                        {student.email} 
                        {student.role && <span style={{ marginLeft: '8px', color: '#4F46E5', fontWeight: 'bold' }}>({student.role})</span>}
                      </ModalStudentEmail>
                    </StudentInfo>
                  </StudentItem>
                ))}
        ...
        <ModalActions>
          ...
          >
            {isLoading ? 'Adding...' : selectedCount > 0 ? `Add ${selectedCount} Student${selectedCount !== 1 ? 's' : ''}` : 'Add Students'}
          </AddButton>
        </ModalActions>
      </ModalContent>
    </ModalOverlay>
...
const EditStudentClassModal: React.FC<EditStudentClassModalProps> = ({
    ...
    return (
        <SectionModalOverlay onClick={onClose}> {/* Re-use SectionModal styles */} 
...
const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({
  ...
  return (
    <SectionModalOverlay onClick={onClose}> {/* Reuse SectionModal styles */}
      <SectionModalContainer onClick={e => e.stopPropagation()} style={{ maxWidth: '450px' }}>
        ...
        <SectionModalFooter style={{ justifyContent: 'space-between' }}>
          <ClassModalCancelButton onClick={onClose} style={{ backgroundColor: '#f3f4f6', color: '#374151' }}>
            Cancel
...
const AssignTeacherModal: React.FC<AssignTeacherModalProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 600 }}>Assign Teacher to {section.name}</h2>
          <button
            onClick={onClose}
            style={{
              background: 'none',
  ...
  return (
    ...
    >
      ...
      <ModalBody>
        ...
        <ButtonGroup>
          <CreateClassCancelButton type="button" onClick={onClose}>Cancel</CreateClassCancelButton>
          <SaveButton type="button" onClick={handleSave}>Create Class</SaveButton>
        </ButtonGroup>
      </ModalBody>
    </Modal>
  );
