...
import {
	...
	AdminInput,
	EnhancedCheckbox,
} from '../../components/admin/StyledFormControls'
import UserForm from '../../components/admin/UserForm'
import { AdminLayout } from '../../components/layout/AdminLayout'
import { Badge, Button, Card, Loader, Pagination } from '../../components/ui'
...
const AdminUsers: React.FC = () => {
	...
	const fetchUsers = async () => {
		try {
			...
			if (usersError) {
				...
				let { data: userData, error: userError } = await supabase.from('user').select('*')
	...
	const handleUserFormSubmit = async (userData: Partial<User>) => {
		...
		if (userData.id) {
			try {
				...
				let { error } = await supabase.from('users').update(updateData).eq('id', userData.id)
			...
			if (userData.id) {
				...
				// Show success message
				showSuccessMessage(
					`User ${userData.firstName} ${userData.lastName} was successfully created`
				)
			}
		}
	...
	// Add function to handle opening the reset password modal
	const handleResetPassword = (user: User) => {
		setUserToResetPassword(user)
		setIsResetPasswordModalOpen(true)
	}

	// Add function to handle closing the reset password modal
	const handleCancelResetPassword = () => {
		setUserToResetPassword(null)
		setIsResetPasswordModalOpen(false)
	}

	// Add function to handle confirming the password reset
	const handleConfirmResetPassword = async () => {
		...
		try {
			...
			// Show success message
			toast.success(`Password reset successfully for ${getDisplayName(userToResetPassword)}`)
							...
							{currentUsers.length > 0 ? (
								currentUsers.map(user => (
									<TR key={user.id}>
										...
										<TD>
											<StatusBadge
												$color={getStatusColor(isUserActive(user) ? 'Active' : 'Inactive')}
											>
										...
										<TD>
											<ActionButtons>
												<ActionButton aria-label='Edit user' onClick={() => handleEditUser(user)}>
					...
					<ModalOverlay>
						<DeleteModal>
							...
							<ModalContent>
								...
								<DeleteWarningText>
									<p>
										Are you sure you want to delete <strong>{selectedUsers.length} users</strong>?
									</p>
									<p>
										This action cannot be undone and will permanently remove the users from the
										system.
									</p>
								</DeleteWarningText>
							</ModalContent>
							...
							<ModalFooter>
								<CancelButton onClick={handleCancelBulkDelete}>Cancel</CancelButton>
								<DeleteButton onClick={handleConfirmBulkDelete} disabled={isBulkDeleting}>
					...
					<ModalOverlay>
						<DeleteModal>
							<ModalHeader>
								...
								<CloseButton onClick={handleCancelResetPassword}>
									<i className='fas fa-times'></i>
								</CloseButton>
							</ModalHeader>
							...
							<ModalContent>
								...
								<DeleteWarningText>
									<p>
										...
										<strong>{getDisplayName(userToResetPassword)}</strong>?
									</p>
									<p>
...
const TR = styled.tr`
	&:hover {
		background-color: ${({ theme }) =>
			theme.isDark ? 'rgba(255, 255, 255, 0.05)' : 'rgba(0, 0, 0, 0.04)'} !important;
	}
`
