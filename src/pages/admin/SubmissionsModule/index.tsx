	...
	useEffect(() => {
		async function getBreadcrumbs() {
			...
			const newBreadcrumbs: Breadcrumb[] = [
				{ name: t('submissions.title'), path: '/admin/submissions', icon: <FiClipboard /> },
			]
			...
			if (parts.includes('levels')) {
				...
				if (levelId) {
					...
					if (levelName) {
						newBreadcrumbs.push({
							name: levelName,
							path: `/admin/submissions/levels/${levelId}/classes`,
							icon: <FiLayers />,
						})
					}
					...
					if (parts.includes('classes')) {
						...
						if (classId) {
							...
							newBreadcrumbs.push({
								name: className,
								path: `/admin/submissions/levels/${levelId}/classes/${classId}/students`,
								icon: <FiUsers />,
							})
							...
							if (parts.includes('students')) {
								...
								if (studentId) {
									...
									newBreadcrumbs.push({
										name: studentName,
										path: `/admin/submissions/levels/${levelId}/classes/${classId}/students/${studentId}`,
										icon: <FiBookOpen />,
									})
								}
							}
			...
			// Check if we're in direct classes navigation
				...
				if (classId) {
					...
					newBreadcrumbs.push({
						name: className,
						path: `/admin/submissions/classes/${classId}/students`,
						icon: <FiUsers />,
					})
					...
					if (parts.includes('students')) {
						...
						if (studentId) {
							...
							newBreadcrumbs.push({
								name: studentName,
								path: `/admin/submissions/classes/${classId}/students/${studentId}`,
								icon: <FiBookOpen />,
							})
						}
					}
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				...
				await submissionsStore.fetchAdminLevels()
				...
				if (parts.includes('levels')) {
					...
					if (levelId) {
						...
						if (parts.includes('classes')) {
							await submissionsStore.fetchAdminLevelClasses(levelId)
					...
					// Direct classes route - fetch all admin classes
					await submissionsStore.fetchAdminClasses()
		...
		<ModuleContainer>
			...
			<ModuleContent>
				<Routes>
					...
					{/* Direct class navigation (admin's view of all classes without level grouping) */}
