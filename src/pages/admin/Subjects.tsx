...
const ClassButton = styled.button<{ $isActive?: boolean }>`
	...
	background-color: ${props =>
		props.$isActive ? props.theme.colors.primary[600] : 'rgba(250, 250, 255, 0.95)'};
	color: ${props => (props.$isActive ? 'white' : 'rgba(55, 65, 81, 1)')};
	border: none;
	cursor: pointer;
	box-shadow: ${props =>
		props.$isActive ? '0 4px 14px rgba(79, 70, 229, 0.25)' : '0 2px 6px rgba(0, 0, 0, 0.06)'};
	position: relative;
	overflow: hidden;
	...
	&:hover {
		background-color: ${props =>
			props.$isActive ? props.theme.colors.primary[500] : 'rgba(245, 245, 255, 1)'};
		transform: translateY(-2px);
		box-shadow: ${props =>
			props.$isActive ? '0 6px 20px rgba(79, 70, 229, 0.3)' : '0 4px 12px rgba(0, 0, 0, 0.08)'};
	}

	&:active {
		transform: translateY(0);
		box-shadow: ${props =>
			props.$isActive ? '0 3px 10px rgba(79, 70, 229, 0.2)' : '0 1px 4px rgba(0, 0, 0, 0.05)'};
	}
...
const WatchButton = styled.button<{ $hasVideo?: boolean }>`
	...
	justify-content: center;
	gap: 0.5rem;
	background-color: ${props => (props.$hasVideo ? props.theme.colors.primary[600] : '#6366f1')};
	color: white;
	border: none;
	border-radius: 0.5rem;
	padding: 0.625rem 1rem;
	...
	&:hover {
		background-color: ${props => (props.$hasVideo ? props.theme.colors.primary[700] : '#4f46e5')};
		transform: translateY(-1px);
	}
		...
		if (data && data.length > 0) {
			...
			if (selectedSubject) {
				...
				const filtered = data.filter(lesson => {
					...
					const subjectId =
						typeof lesson.subjectid === 'object' ? lesson.subjectid?.id : lesson.subjectid
...
const Subjects: React.FC = () => {
	...
	// State for subjects and lessons
	...
	const [newlyCreatedSubjectId, setNewlyCreatedSubjectId] = useState<string | null>(null)
	...
	const confirmDeleteSubject = async () => {
		...
		try {
			...
			console.log(
				`Timetable reference check returned: ${
					timetableReferences ? timetableReferences.length : 0
				} results.`
			)
			...
			if (timetableReferences && timetableReferences.length > 0) {
				...
				toast.error(
					'Cannot delete subject: It is currently used in the timetable. Please remove it from the schedule first.'
				)
			...
			if (associatedLinks && associatedLinks.length > 0) {
				const associatedClassIds = associatedLinks
					.map(link => link.classid)
					.filter(id => id != null) // Get non-null class IDs
				...
				if (associatedClassIds.length > 0) {
					...
					console.log(
						`Class timetable reference check returned: ${
							classTimetableRefs ? classTimetableRefs.length : 0
						} results.`
					)
					...
					if (classTimetableRefs && classTimetableRefs.length > 0) {
						...
						toast.error(
							'Cannot delete subject: A class associated with this subject is still scheduled. Please remove related timetable entries first.'
						)
						setShowDeleteSubjectModal(false)
						setSubjectToDelete(null)
						setLoading(false)
			...
			setFilteredSubjects((prev: Subject[]) =>
				prev.filter((s: Subject) => s.id !== subjectToDelete)
			)
			...
			if (showDeleteSubjectModal) {
				// Check if modal is still technically open
				setShowDeleteSubjectModal(false)
				setSubjectToDelete(null)
			}
		}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (isEditMode && selectedSubject) {
				...
				const { data, error } = await updateSubject(selectedSubject.id, formData)
				...
				// Create new subject
				const { data, error } = await addSubject(formData as Omit<Subject, 'id'>)
				...
				if (data && data.length > 0) {
					...
					// Store the newly created subject ID
					setNewlyCreatedSubjectId(data[0].id)

					toast.success('Subject created successfully')

					// Show assign modal after creating a new subject
					setShowModal(false)
					setTimeout(() => {
			...
			// Reset form and close modal if not showing assign modal
			if (isEditMode) {
				setShowModal(false)
				setFormData({
					subjectname: '',
	...
	const handleLessonInputChange = (
		...
		const { name, value } = e.target
		setLessonFormData((prev: { lessonname: string; description: string; videourl: string }) => ({
			...prev,
			[name]: value,
		}))
	}
		...
		try {
			...
			setLessons(prevLessons =>
				prevLessons.map(lesson =>
					lesson.id === currentVideoLesson.id ? { ...lesson, videourl: videoUrl } : lesson
				)
			)
	...
	const handleLessonEditSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (data && data[0]) {
				...
				setFilteredLessons(prev => prev.map(l => (l.id === updatedLesson.id ? updatedLesson : l)))
	...
	const handleDeleteLesson = async () => {
		...
		setLoading(true)
		try {
			await supabase.from('lessons').delete().eq('id', lessonToDelete)
	...
	const handleAssignSubjectToGrade = async (
		...
		// If we have a newly created subject and no subjects were selected,
		// automatically add the newly created subject to the selection
		if (newlyCreatedSubjectId && subjectIds.length === 0) {
			subjectIds = [newlyCreatedSubjectId]
		}
		...
		try {
			...
			if (!sections || sections.length === 0) {
				toast.info(
					`No class sections found for Grade ${grade} (Level ID: ${levelId}). No subjects assigned.`
				)
			...
			console.log(
				`Found ${sections.length} sections for level ID ${levelId}:`,
				sections.map(s => s.classname)
			)
			...
			for (const subjectId of subjectIds) {
				...
				if (upsertError) {
					...
					console.log(
						`Subject ${subjectId}: Inserted ${insertedCount}, Ignored ${
							relationships.length - insertedCount
						}`
					)
				}
			...
			console.log(
				`Assignment Summary: Total Attempted=${totalRelationshipsAttempted}, Success=${successCount}, Errors=${errorCount}, Already Existed/Ignored=${alreadyAssignedCount}`
			)

			if (errorCount > 0 && successCount > 0) {
				toast.warning(
					`Assigned ${successCount} new subject relationships with ${errorCount} errors. ${alreadyAssignedCount} were already present.`
				)
				...
				toast.success(
					`Successfully assigned subjects to ${successCount} section(s) of Grade ${grade}. ${alreadyAssignedCount} relationships already existed.`
				)
			} else {
		...
		>
			...
			{selectedSubject ? (
				...
				<LessonsContainer>
					<LessonsHeader>
						<LessonsTitle>
							{t('subjectManagement.lessonsFor', { subjectName: selectedSubject.subjectname })}
						</LessonsTitle>
						...
						<LessonsList>
							{filteredLessons.map(lesson => (
								<LessonItem key={lesson.id}>
									<LessonDate>
										{lesson.uploadedat
											? formatDate(lesson.uploadedat)
											: t('subjectManagement.noDateAvailable')}
									</LessonDate>
									<LessonTitle>
									...
									<LessonActionsContainer>
										<WatchButton
											onClick={e =>
												lesson.videourl ? handleWatchVideo(lesson, e) : handleUploadVideo(lesson, e)
											}
											$hasVideo={!!lesson.videourl}
										>
											<FiPlay />
											{lesson.videourl
												? t('subjectManagement.watchVideo')
												: t('subjectManagement.uploadVideo')}
										</WatchButton>
										<EditButton onClick={e => handleEditLesson(lesson, e)}>
											<FiEdit2 />
											{t('subjectManagement.edit')}
										</EditButton>
										<DeleteButton onClick={e => handleShowDeleteConfirmation(lesson.id, e)}>
											<FiTrash2 />
				...
				<>
					...
					{loading && selectedLevel ? (
						...
						<ChooseSubjectMessage>{t('subjectManagement.chooseSubject')}</ChooseSubjectMessage>
					)}
				</>
			)}
						...
						<form onSubmit={handleSubmit}>
							...
							<FormButtonContainer>
								...
								<FormButton type='submit' disabled={loading}>
									{isEditMode
										? t('subjectManagement.updateSubject')
			...
			{isLessonModalOpen && (
				<ModalOverlay initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
					<ModalContent initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
						<ModalHeader>
							<ModalTitle>{t('subjectManagement.addNewLesson')}</ModalTitle>
			...
			{isLessonEditModalOpen && (
				<ModalOverlay initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
					<ModalContent initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
						<ModalHeader>
							<ModalTitle>{t('subjectManagement.editLesson')}</ModalTitle>
						...
						<form onSubmit={handleLessonEditSubmit}>
							...
							<FormGroup>
								<FormLabel htmlFor='edit-description'>
									{t('subjectManagement.description')}
								</FormLabel>
								<FormTextarea
									id='edit-description'
									name='description'
			...
			{showDeleteConfirmation && (
				<ModalOverlay initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
					...
					>
						<ModalHeader>
							<ModalTitle>{t('subjectManagement.confirmDeletion')}</ModalTitle>
						...
						<div style={{ padding: '1rem', textAlign: 'center' }}>
							<p style={{ marginBottom: '1.5rem' }}>{t('subjectManagement.deleteLessonConfirm')}</p>
			...
			{showVideoModal && (
				<ModalOverlay initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
					<ModalContent initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
						<ModalHeader>
							<ModalTitle>{t('subjectManagement.addVideoUrl')}</ModalTitle>
			...
			<AssignSubjectModal
				...
				newlyCreatedSubjectId={newlyCreatedSubjectId}
			/>
