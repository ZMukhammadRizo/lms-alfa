...
export async function debugCheckTables() {
	...
	try {
		// Check scores table
		const { data: scores, error: scoresError } = await supabase.from('scores').select('*').limit(1)
...
export async function getStudentGrades(studentId: string): Promise<SubjectGrade[]> {
	try {
		...
		const combinedSubjects = classSubjectsData
			.map(classSubject => {
				...
				try {
					...
					if (lessonIds.length > 0) {
						try {
							...
							if (scoresError) {
								console.error(
									`❌ Error fetching scores for subject ${subjectId} lessons:`,
									scoresError
								)
							} else if (rawScores) {
								...
								console.log(
									`💰 Mapped ${scoresData?.length || 0} score records for subject ${subjectId}`
								)
							} else {
					...
					if (lessonIds.length > 0) {
						try {
							...
							if (attendanceError) {
								console.error(
									`❌ Error fetching attendance for subject ${subjectId} lessons:`,
									attendanceError
								)
								...
								console.log(
									...
									} attendance records for subject ${subjectId}`
								)
							}
						} catch (err) {
						...
						grades = quartersData.map(quarter => {
							...
							const averageScore =
								quarterScores.length > 0
									? Math.round(
											quarterScores.reduce((sum, score) => sum + score.score, 0) /
					...
					*/
					...
					const color =
						subject && subject.subjects ? getSubjectColor(subject.subjects.subjectname) : '#4299E1'

					return {
						id: subjectId,
						subjectName:
							subject && subject.subjects ? subject.subjects.subjectname : 'Unknown Subject',
						teacherName: `${teacherFirstName} ${teacherLastName}`,
						teacherFirstName: teacherFirstName,
						teacherLastName: teacherLastName,
						className:
							subject && subject.classes
								? subject.classes.classname || 'Unknown Class'
								: 'Unknown Class',
						color,
						grades,
						dailyScores,
...
function calculateAttendance(attendanceData: any[]): {
	...
	attendanceData.forEach(record => {
		...
		switch (status) {
			...
			case 'excused':
				...
				break
			case 'not-assigned':
				attendance.notAssigned++
				break
			default:
				// Handle unknown or null statuses - treat as not-assigned if no status is set
				if (!status) {
					attendance.notAssigned++
				} else {
	...
	// Calculate attendance percentage - only include assigned days (exclude not-assigned)
	const assignedTotal =
	...
	attendance.percentage =
		assignedTotal > 0 ? Math.round((presentEquivalent / assignedTotal) * 100) : 0
...
export function getMockGrades(): SubjectGrade[] {
	const subjects = [
		{
			id: '1',
			name: 'Mathematics',
			teacherFirst: 'Richard',
			teacherLast: 'Thompson',
			class: 'Class 10-A',
			color: '#4299E1',
		},
		{
	...
	return subjects.map(subject => {
		...
		const notAssigned = Math.floor(Math.random() * 8) + 2 // 2-9 days not assigned
		const assignedTotal = present + absent + late + excused // Only assigned days for percentage
		const percentage = Math.round((present / assignedTotal) * 100)
			...
			dailyScores: [
				{
					...
					lessonDate: '2024-01-10T10:00:00Z',
					score: 9,
					quarterId: '1',
				},
				{
					...
					lessonDate: '2024-01-17T11:30:00Z',
					score: 10,
					quarterId: '1',
				},
				{
					id: 'ds3',
					lessonId: 'l3',
					lessonTitle: 'Geometry Basics',
					lessonDate: '2024-03-05T09:15:00Z',
					score: 7,
					quarterId: '3',
				},
			],
...
export const getSubjectGradeDetails = async (
	studentId: string,
	subjectId: string
): Promise<SubjectGradeDetails> => {
	console.log(
		`[gradesService] Fetching grade details for student ${studentId}, subject ${subjectId}`
	)
	try {
		...
		const gradeDetails = assignments.map((assignment): AssignmentGradeDetail => {
			...
			return {
				...
				gradedAt: scoreEntry?.updated_at
					? new Date(scoreEntry.updated_at).toLocaleDateString()
					: '-',
			}
		})
